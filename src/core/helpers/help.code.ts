...
class CodeHelper {
    ...
    findMatchingIndex(fileContentLines: string[], oldBlock: string[]): number {
        ...
        for (let i = 0; i <= length - oldBlock.length; i++) {
            if (fileContentLines.slice(i, i + oldBlock.length).every((line, index) => line.trim() === oldBlock[index].trim())) {
                return i;
            }
        }
     ...
     */
    replaceOldCodeWithNew(fileContentLines: string[], oldBlock: string[], newBlock: string[], matchIndex: number): string[] {
        if (matchIndex !== -1) {
