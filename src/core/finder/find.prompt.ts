...
class FindPrompt {
     ...
     */
    async handleFoundAcceptance(acceptanceLine: string, codeBlock: string, response: string, fileContent: string, filePath: string): Promise<void> {
     ...
     */
    async handleFoundPrompt(index: number, fileContent: string, prompt: string, filePath: string, verbose: boolean): Promise<void> {
        try {
            ...
            for (const caseItem of promptCases) {
                switch (caseItem.type) {
                    case 'prompt':
                        ...
                        await this.handleFoundPrompt(caseItem.lineIndex, fileContent, caseItem.content, filePath, verbose);
                        break;
                    case 'acceptance':
                        if (verbose) {
