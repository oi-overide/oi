...
class Config extends OiCommand {
    ...
    configureCommand(): void {
        const configCommand = this.program.command('config').description('Update Local or Global settings')
    ...
    async handleGlobalConfig(verbose = false) {
        ...
        if (verbose) {
            console.log(`${CommandHelper.configExists(true) ? 'Updated' : 'Created'} config at: ${configFilePath}`)
        };
